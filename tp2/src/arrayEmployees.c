...
static int employee_searchEmpty(Employee list[], int limit);
...
int employee_addEmployee(Employee *list, int len)
{
	...
	if(list != NULL && len > 0)
	{
		if(employee_searchEmpty(list, len) >= 0 && utn_getString("Ingrese el nombre del empleado: ", "\nERROR! Ingrese un nombre valido: ", bufferEmployee.name, 2, NAME_SIZE) == 0 &&
							...
							"\nERROR! Elija una opcion valida",	&choosenOption, 2, 1, 5) == 0)
					{
						switch(choosenOption)
						{
							case 1:
								...
								{
									printf("\nNombre del empleado modificado con exito! Nuevo nombre: %s", list[index].name);
									retornar = 0;
								}
								else
								{
							...
							case 2:
								if(utn_getString("Ingrese el nuevo apellido del empleado: ", "\nERROR! Ingrese un apellido valido: ",
													list[index].lastName, 2, len) == 0)
								{
									printf("\nApellido del empleado modificado con exito! Nuevo apellido: %s", list[index].lastName);
									retornar = 0;
								}
								else
								{
							...
							case 3:
								if(utn_getFloat("Ingrese el nuevo salario del empleado: ", "\nERROR! Ingrese un salario valido: ",
													&list[index].salary, 2, SALARY_MIN, LONG_MAX) == 0)
								{
									printf("\nSalario del empleado modificado con exito! Nuevo salario: %.2f", list[index].salary);
									retornar = 0;
								}
								else
								{
							...
							if(utn_getInt("Ingrese el nuevo sector del empleado: ", "\nERROR! Ingrese un sector valido: ",
													&list[index].sector, 2, 0, 500) == 0)
								{
									printf("\nSector del empleado modificado con exito! Nuevo sextor: %d", list[index].sector);
									retornar = 0;
								}
								else
								{
...
int employee_report(Employee *list, int len)
{
	...
	if(list != NULL && len > 0 && searchForEmpty(list, len) == 0)
	{
		if(utn_getInt("\nQue opcion desea elegir?\n1)Listado de los empleados ordenados por apellido."
				"\n2)Total y promedio de los salarios, y cuántos empleados superan el salario promedio\n",
				"\nERROR! Ingrese una opcion valida", &option, 2, 1, 3) == 0)
		{
			switch(option)
			{
			case 1:
			...
			break;
			case 2:
				if(salaryTotal(list, len, &totalSalary, &averageSalary) == 0 && surpassAverage(list, len, averageSalary, &surpassAverageCounter) == 0)
				{
 ...
 */
static int employee_searchEmpty(Employee *list, int len)
{
	int retornar = -1;
	if(list != NULL)
	{
